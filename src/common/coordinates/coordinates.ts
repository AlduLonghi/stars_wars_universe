export class Coordinates {
    public coordinatePattern = /^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?),\s*[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d))(\.\d+)?)$/;

    private toLongitudAndLatitud(distance1:string, distance2:string) {
      let [latitude1, longitude1] = distance1.split(',').map(parseFloat);
      let [latitude2, longitude2] = distance2.split(',').map(parseFloat);
      
      return { latitude1, longitude1, latitude2, longitude2 }
    }
    
    calculateGalacticDistance(distance1:string, distance2:string): number {
        let { latitude1, longitude1, latitude2, longitude2 } = this.toLongitudAndLatitud(distance1, distance2);
